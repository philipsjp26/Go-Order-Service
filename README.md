...
# example
$ docker build -t order_service -f deployment/dockerfiles/dockerfile .
```

### run docker container after build the image
```bash
# example run http serve:
$ docker run -i --name order_service -p 8081:8081 -t order_service
...
mkdir $HOME/go/pkg
mkdir $HOME/go/bin
mkdir -p $HOME/go/src/gitlab.privy.id/order_service
chmod -R 775 $HOME/go
cd $HOME/go/src/gitlab.privy.id/order_service
...
# run command :
cd $HOME/go/src/gitlab.privy.id/order_service
git clone -b development https://gitlab.privy.id/order_service .
cd $HOME/go/src/gitlab.privy.id/order_service
